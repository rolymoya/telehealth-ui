...
describe("get started page", () => {
  it("renders as a static start shell without clinical fields", () => {
    ...
    expect(screen.getByRole("heading", { name: "Start with a secure account." }))
      .toBeInTheDocument();
    expect(screen.getByText(/Clinical intake happens after account setup/i))
      .toBeInTheDocument();
    expect(screen.queryByLabelText(/condition|medication|symptom|diagnosis|questionnaire/i))
      .toBeNull();
  });

  it("shows account creation as the primary signed-out path", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Create account" }))
  ...
  it("shows a retry state for provider failures without clinical fields", async () => {
    ...
    expect(await screen.findByText(/could not check your account status/i))
      .toBeInTheDocument();
