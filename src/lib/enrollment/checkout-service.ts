...
import type {
  EnrollmentRepository as EnrollmentPersistenceRepository,
...
export async function beginEnrollmentCheckout(input: {
  ...
  repository: EnrollmentPersistenceRepository;
  ...
  if (!safeHostedCheckoutSession(session)) {
    await markCheckoutRetryable(input.repository, {
...
function enrollmentCheckoutUrls(input: {
  ...
  try {
    ...
    return {
      ...
      successUrl:
        `${successOrigin.origin}/api/enrollment/checkout-return?session_id={CHECKOUT_SESSION_ID}`,
    };
  } catch {
    return null;
  }
...
function safeHostedCheckoutSession(
