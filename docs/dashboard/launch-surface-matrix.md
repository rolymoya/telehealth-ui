...
| Refills and follow-up care | Deferred by default; API-backed status mirror only after T-061 validation | MDI refill/care workflow, case/order status, patient subscription/case reads where safe | No embedded refill route approved for launch; status/cue reads may use `partner-get-partner-cases-case-id-orders-get-case-orders` and `partner-get-partner-cases-case-id-offerings-get-case-offerings` after T-061 validation | `open_mdi_refills`, `follow_up_pending`, `refills_deferred` | Opaque case/order/workflow pointer, bounded status code, timestamp | Medication names, dose, directions, pharmacy notes, prescription details, refill reason/free text | Deferred unless T-061 validates an MDI-owned route; iframe deferred | Until validated, show only `refills_deferred` and route to support | T-061, T-062, T-063 |
...
- T-061: confirm refill and follow-up workflow source. Until then, the dashboard
  may show only a generic `refills_deferred` or `open_mdi_refills` action when
  an approved MDI workflow exists.
