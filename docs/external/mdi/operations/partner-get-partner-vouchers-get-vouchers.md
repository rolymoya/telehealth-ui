...
- Operation slug: `partner-get-partner-vouchers-get-vouchers`
- Surface: `partner`
- Method: `GET`
- Path: `/partner/vouchers/`
- Raw URL template: `{{baseUrl}}/partner/vouchers/?expired=REDACTED_SCALAR`
...
- data[].payload: object
- data[].payload.diseases: array
- data[].payload.environment_id: string
- data[].payload.hold_status: boolean
- data[].payload.offerings: array
- data[].payload.offerings[]: object
- data[].payload.offerings[].force_pharmacy: boolean
- data[].payload.offerings[].offering_id: string
- data[].payload.offerings[].pharmacy_id: number
