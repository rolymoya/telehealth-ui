...
- Added `src/lib/__tests__/sqs.test.ts` covering:
  - webhook queue config resolution and missing-setting failures,
  - signed SQS `SendMessage` request shape,
  - session-token SigV4 header inclusion,
  - queue message body contract using opaque webhook IDs only,
  - SQS non-OK response propagation for retry handling.
...
- Earlier in the same thread, a targeted batch completed and committed:
  - `f8e6c8c feat: add Stripe webhook receiver (T-026)`
  - `eb36b81 fix: load runtime secrets from Secrets Manager (ISS-005)`
  - `c65ca1d fix: scope startup secret checks to runtime routes (ISS-008)`
...
- Good backend candidates to inspect next: ISS-020 (simplify webhook clock/now API), ISS-021 (processing lease seconds through repository claim), or ISS-012 (shared observability metric contract cleanup).
